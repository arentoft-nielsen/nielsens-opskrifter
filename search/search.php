 ...
 *           RiSearch PHP
 * 
 * web search engine, version 0.2 (c) Sergej Tarasov, 2000-2004
 * 
...
$stype = "AND";
$query = "";
$queryFound = false;  // TAN
$abort = 0;

if (isset($_GET["query"]) and preg_replace("/[\d\s]/", "", $_GET["query"]) != '') { // TAN tjek om der er søgestreng efter vi fjerner tal og whitespace
	
	$queryFound = true; // Hvis ja, så true her
	get_query();
	...
	if (count($query_arr) > 0) {
    	get_results();
			...
			$time3 = getmicrotime();
			$time = $time3-$time1;
			// print "<BR>get_results() tog $time sek.<BR>";
		...
		boolean();
			...
			// print "<BR>boolean() tog $time sek.<BR>";
	}
 }

print print_template("header");

if ($queryFound == true && count($query_arr) > 0) { // TAN - indsat tjek for ok søgestreng, ellers vil count() fejle
    if ($rescount > 0) {
...
function get_results() {
    ...
    $fp_HASH = fopen ("$HASH", "rb") or die("Indeksfil mangler! Kør indeksering igen.");
    $fp_HASHWORDS = fopen ("$HASHWORDS", "rb") or die("Indeksfil mangler! Kør indeksering igen.");
    $fp_SITEWORDS = fopen ("$SITEWORDS", "rb") or die("Indeksfil mangler! Kør indeksering igen.");
    $fp_WORD_IND = fopen ("$WORD_IND", "rb") or die("Indeksfil mangler! Kør indeksering igen.");
